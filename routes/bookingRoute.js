...
const FurnitureBooking = require('../models/bookingModel');
const Product = require('../models/productModel');
const auth = require('../middleware/auth');
const {todayDate,bookingData} = require('../utils/utils');
const {check,validationResult} = require('express-validator')

router.post('/book/furniture',auth.verifyUser,(req,res)=>{
    ...
    FurnitureBooking.findOne({"product_id":pid,"user_id":req.user._id}).then((furniture)=>{
        if(furniture == null)
        {
            FurnitureBooking.find({}).then((data)=>{
                    ...
                    Product.findOne({"_id":pid}).then((data2)=>{
                        if(data2!=null)
                        {
                            if(data2.availableStock >= quantity)
                            {
                                ...
                                const booking = new FurnitureBooking({"product_id":pid,"quantity":quantity,"price":price,"booked_At":booked_At,"deliveryStarts":deliveryStarts,"delivery_address":address,"delivery_number":phone,"bookingCode":bookingCode,"user_id":req.user._id,bookedDate:new Date()});
...
router.get('/retrieve/myBookings',auth.verifyUser,(req,res)=>{
    ...
    let query = FurnitureBooking.find({"user_id":req.user._id}).sort({"bookedDate":-1}).populate(
        {
            path:"product_id"
        }
    );
...
router.post('/delete/booking',auth.verifyUser,(req,res)=>{
    ...
    let query = FurnitureBooking.findOne({"_id":pid});
    query.then((data)=>{
        if(data!=null)
        {
            let query3 = FurnitureBooking.findOne({"_id":pid});
            query3.then((data3)=>{
                if(data3!=null)
                {
            ...
            query2.then((data2)=>{
                if(data2 != null)
                {
                    ...
                    Product.updateOne({"_id":product_id},{$set:{"availableStock":data2.availableStock,"sold":data2.sold}}).then((result)=>{
                        FurnitureBooking.deleteOne({"_id":pid}).then((result2)=>{
...
router.post('/update/booking',auth.verifyUser,(req,res)=>{
    ...
    FurnitureBooking.findOne({"_id":pid}).then((data)=>{
        if(data!=null)
        {
            FurnitureBooking.findOne({"_id":pid}).then((data2)=>{
                if(data2!=null)
                {
                    ...
                    Product.findOne({"_id":product_id}).then((data3)=>{
                      if(data3!=null)
                      {
                        ...
                        if(data3.availableStock >= qty)
                        {
                            ...
                            FurnitureBooking.updateOne({"_id":pid},{$set:{"quantity":qty,"price":price}}).then((result2)=>{
