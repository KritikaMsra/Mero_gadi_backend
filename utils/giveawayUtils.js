...
const Registrations = require('../models/user_register_model');
            ...
            .then((data2)=>{
                if(data2.length > 0)
                {
                    for(var l of data2)
                    {
                        ...
                        sendMailMessage("FunFurnish",l.email,content);
                    }
        ...
        .then((data)=>{
            if(data != null)
            {
                ...
                let content = {
                    ...
                    "message3":reward[data.userName],
                   
                    "task":"Giveaway3"
                }
              
                 sendMailMessage("FunFurnish",data.email,content);
            }
        })
        .catch((err)=>{
            return res.status(404).json({"success":false,"message":err});
...
const giveawayResult = (req,res)=>{
    TimeHalt.findOne({"_id":"60e9513c6ddca565c5a0cdf2","updated_at":{$ne:getFormattedToday(new Date())}})
    .then((halt)=>{
        if(halt != null)
        {
            TimeHalt.updateOne({"_id":"60e9513c6ddca565c5a0cdf2"},{$set:{"updated_at":getFormattedToday(new Date())}})
                .then((result)=>{
                                ...
                                query3.then((data3)=>{
                                    ...
                                    for(l of data3)
                                    {
                                        ...
                                        .then((reg_user)=>{
                                            ...
                                            if(reg_user != null)
                                            {
                                                let query4 = FurnitureBooking.find({"user_id":l.user_id._id}).countDocuments({});
                                            query4.then((count)=>{
                                                
                                                if(count >= 15)
                                                {
                                                    categorization["regular"].push(reg_user.username)
                                                }
                                                else
                                                {
